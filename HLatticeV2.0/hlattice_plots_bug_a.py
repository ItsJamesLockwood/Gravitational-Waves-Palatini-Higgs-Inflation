...
eliot_file = r"data/Lphi4_LatticeEasy_128points_Run1_screen.log"
eliot_file2=r"data\Dataset-Friday_screen.log"
higgs_file = "data/higgs-vev-run1_screen.log"
tanhfile = "data/higgs-tanh4-run1_screen.log"

filefile = eliot_file
filefile2=eliot_file2
...
def n_k(pw_data1,pw_data2,L=64):
    ...
    ns.columns = ks
    print("this is data frame ns")
    print(ns)#Add a values back for plotting purposes
    ns.insert(len(ns.columns),'a',a_list)
...
#take in data with 71 values
pw_data1, pw_data2 = import_pw('data/'+trim_name(filefile) + '_pw_%i.log'%pw_field_number)
#take in 35 values
pw_data1_1,pw_data2_1=import_pw('data/'+trim_name(filefile2) + '_pw_%i.log'%pw_field_number)
#use function to calculate data of 128 points
n_df = n_k(pw_data1,pw_data2)
cb=n_df.copy(deep=True)
#generate 35 k values
ks=k_list(pw_data1_1)
#calculated from fridays set 64 points
n_df_2=n_k(pw_data1_1,pw_data2_1)
#need to rearrange from the fact there is 2x more a values data in the new set 
n_df=n_df.iloc[0:214,:]
vb=n_df.copy(deep=True)
n_df=n_df.iloc[::2,:]
n_df.drop(n_df.iloc[:, 35:72], inplace = True, axis = 1)
n_df.reset_index(drop=True,inplace=True)
#take out a from n_df_2 to make them the exact same frame
n_df_2.drop(columns=['a'], inplace=True)
#define difference between calculated values
n_df_diff=(n_df-n_df_2)/n_df_2
# define plotting function
#plot difference vs k for different values of a
def plot_this(ks,dataframes,rows=[]):
   #make list of list from data frame and plot
    df_list = dataframes.values.tolist()
    if rows==[]:
        rows.append(int(ns.shape[0]/2))
    colors = np.flip(cm.magma(np.linspace(0,1,len(rows))),axis=0)
    plt.yscale('log')
    for i in range(len(rows)):
        plt.xlabel("k values")
        plt.ylabel("Difference between measurement methods")
        plt.plot(ks/2/np.pi,df_list[rows[i]],label=a_list[rows[i]],color=colors[i])
    plt.legend()
rows=[1,10,20,30,40,50,60,70,80,90]
my_rows = np.searchsorted(cb['a'],rows)
#plot_this(ks,np.abs(n_df_diff),rows=my_rows)
plot_fig6(cb, rows=my_rows)
