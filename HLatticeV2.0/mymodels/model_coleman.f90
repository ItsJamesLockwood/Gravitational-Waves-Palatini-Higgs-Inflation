!! This model is still as WIP. Not currently functioning.
...
!!the predefined constants that you can use: GeV, MPl (the reduced Planck Mass), PlanckMass (the Planck Mass), Mplsq (square of the reduced Planck Mass)
  real(dl):: A =1.60d-14 * PlanckMass
  real(dl),parameter:: M = 1.d0 * PlanckMass
  real(dl),parameter:: M4 = M**4
  real(dl),parameter:: p0 = 1.1d0
...
!! the potential of the scalar fields
  function potential(f)
    ...
    real(dl) potential
    if(PHI.le.0) then
      stop "Negative field value should not occur in this model"
    end if 
    potential = (A * PHI**4) * ( log(PHI/M) - .25d0) + A*M4/4.d0 
...
!!d^2 V / d f_{fld}^2,, here fld can be 1,2,..., ns
  function mass_sq(f,fld)
    ...
    case(1)
        ...
        mass_sq = 4.d0*A*PHI**2 * (3.d0* log(PHI/M) + 1)
    case default
  ...
  function model_Power(fld,k)
    ...
    if(k.gt.Init_Hubble)then 
      ...
      !We now consider the tachyonic region (i.e. k_min < k < k_max)
         ...
         model_Power(1) = 1.d0/k /metric%a**3
         model_Power(2) = k/metric%a**3
         if(warning)then
