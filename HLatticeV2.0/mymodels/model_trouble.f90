...
!! the potential of the scalar fields
  function potential(f)
    ...
    real(dl) potential
    real(dl) dummy, d2
    dummy = (lambda * Mplsq / 4.d0 /xi2 ) * TANH( xisqrt * PHI / Mpl)**4 
    write(*,*) "Dummy value: ",dummy
    d2 = 4 * (lambda * Mplsq / 4.d0 /xi2 ) * xisqrt/Mpl * TANH( xisqrt * PHI / Mpl )**3 / COSH(xisqrt * PHI / Mpl)**2
    write(*,*) "dVdf value: ",d2
