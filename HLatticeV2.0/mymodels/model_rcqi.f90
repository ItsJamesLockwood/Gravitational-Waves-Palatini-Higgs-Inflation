!!The model here is V=lambda/4 phi^4 with radiative corrections
module model
  use define_fields
  implicit none
...
!! the potential of the scalar fields
  function potential(f)
    real(dl) f(ns)
    real(dl) potential
    potential = (lambda/4.d0) * PHI**4  * ( 1 - alpha * log(PHI/Mpl) ) 
...
!! return an array (dV/df1, dV/df2, ...)
  function dVdf(f)
    ...
    dVdf = (/ &
         lambda*PHI**3 * (1._dl - alpha/4 - alpha* log(PHI/Mpl))    &  
         /)
  end function dVdf
...
!!d^2 V / d f_{fld}^2,, here fld can be 1,2,..., ns
  function mass_sq(f,fld)
    ...
    select case(fld)
    case(1)
       mass_sq = 3.d0*lambda* PHI**2 * ( 1._dl - 7.d0*alpha/12 - alpha*log(PHI/Mpl))
    case default
